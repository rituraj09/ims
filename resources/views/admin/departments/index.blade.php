...
@section('content')
    <div class="card">
                    ...
                    <tbody>
                        @forelse($departments as $i => $dept)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="status-pill text-{{ $dept->status === 'active' ? 'success' : 'danger' }} bg-{{ $dept->status === 'active' ? 'success' : 'danger' }} bg-opacity-10">
                                        {{ ucfirst($dept->status) }}
                                    </span>
                                </td>
                                <td>
                                        ...
                                        @can('departments.delete')
                                            <form action="{{ route('admin.departments.destroy', $dept) }}" method="POST"
                                                ...
                                                <button type="submit" class="btn btn-icon btn-sm btn-outline-danger"
                                                    data-confirm="Delete '{{ $dept->name }}'?" data-bs-toggle="tooltip"
                                                    title="Delete">
        ...
        @if ($departments->hasPages())
            <div class="card-footer">{{ $departments->links('pagination::bootstrap-5') }}</div>
        @endif
    </div>
@endsection
