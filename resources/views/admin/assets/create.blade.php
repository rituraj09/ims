...
<form action="{{ route('admin.assets.store') }}" method="POST"
      enctype="multipart/form-data" class="needs-validation" novalidate id="assetForm">
@csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="asset_type" class="form-select">
                        ...
                        <option value="Movable"    {{ old('asset_type') === 'Movable'    ? 'selected' : '' }}>Movable</option>
                        <option value="Immovable"  {{ old('asset_type') === 'Immovable'  ? 'selected' : '' }}>Immovable</option>
                        <option value="IT"         {{ old('asset_type') === 'IT'         ? 'selected' : '' }}>IT Equipment</option>
                        <option value="Non-IT"     {{ old('asset_type') === 'Non-IT'     ? 'selected' : '' }}>Non-IT Equipment</option>
                ...
                <div class="col-md-6">
                    ...
                    <select name="category_id" id="categorySelect"
                        ...
                        <option value="{{ $cat->id }}"
                                data-rate="{{ $cat->depreciation_rate }}"
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="model" class="form-control"
                           value="{{ old('model') }}" placeholder="e.g. Latitude 5520">
                </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="purchase_price"
                               ...
                               placeholder="0.00" step="0.01" min="0" id="purchasePrice">
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        <input type="number" name="depreciation_rate"
                               id="depreciationRate" class="form-control"
                ...
                <div class="col-12">
                    <div class="form-check form-switch">
                        <input type="checkbox" name="under_amc" class="form-check-input"
                               id="underAmc" value="1"
                               {{ old('under_amc') ? 'checked' : '' }}>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="assigned_to_type" id="assignedToType" class="form-select">
                        ...
                        <option value="department" {{ old('assigned_to_type') === 'department' ? 'selected' : '' }}>Department / Branch</option>
                        <option value="employee"   {{ old('assigned_to_type') === 'employee'   ? 'selected' : '' }}>Employee</option>
                    </select>
                </div>
                ...
                <div class="col-md-6" id="assignedOnField">
                    ...
                    <input type="text" name="assigned_on"
                           class="form-control datepicker"
                ...
                <div class="col-md-6 d-none" id="deptField">
                    ...
                    <select name="assigned_department_id" class="form-select select2">
                        ...
                        <option value="{{ $dept->id }}" {{ old('assigned_department_id') == $dept->id ? 'selected' : '' }}>
                            {{ $dept->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea name="assignment_notes" class="form-control" rows="2"
                              placeholder="Any notes about assignment...">{{ old('assignment_notes') }}</textarea>
                </div>
...
$('#categorySelect').on('change', function () {
    ...
    // Generate asset tag
    if (catId) {
        $.get('{{ route("admin.assets.generate-tag") }}', { category_id: catId }, function (r) {
...
function updateSummary() {
    ...
    $('#sumPurchasePrice').text('₹ ' + price.toLocaleString('en-IN', {minimumFractionDigits:2}));
    $('#sumDepRate').text(rate ? rate + ' %' : '— %');

    if (price && rate) {
        const years   = 1; // First year estimate
        const current = price * Math.pow(1 - rate/100, years);
        $('#sumCurrentValue').text('₹ ' + current.toLocaleString('en-IN', {minimumFractionDigits:2}));
    } else {
