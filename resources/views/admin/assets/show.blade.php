...
<div class="d-flex gap-2 flex-wrap">
    ...
    @endcan
    <a href="{{ route('admin.assets.print', $asset) }}" class="btn btn-outline-secondary btn-sm" target="_blank">
...
<div class="alert border-0 mb-3 d-flex align-items-center gap-3 py-3"
    ...
    <div class="d-flex align-items-center gap-2 flex-wrap flex-grow-1">
        @php
            $statusColors = ['available'=>'success','in_use'=>'primary','under_maintenance'=>'warning','disposed'=>'danger','lost'=>'dark','transferred'=>'info'];
            $condColors   = ['new'=>'success','good'=>'info','fair'=>'warning','poor'=>'danger','condemned'=>'dark'];
        @endphp
        <span class="status-pill text-{{ $statusColors[$asset->status]??'secondary' }} bg-{{ $statusColors[$asset->status]??'secondary' }} bg-opacity-10 fs-6">
            <i class="fas fa-circle-dot me-1"></i>{{ $asset->status_label }}
        </span>
        <span class="badge bg-{{ $condColors[$asset->condition]??'secondary' }} bg-opacity-10 text-{{ $condColors[$asset->condition]??'secondary' }}">
        ...
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <div class="fw-500 {{ $asset->warranty_expiry_date && $asset->warranty_expiry_date->isPast() ? 'text-danger' : '' }}">
        ...
        <div class="card-body">
            ...
            <div class="d-flex align-items-center gap-3 mb-3">
                <div style="width:44px;height:44px;border-radius:12px;background:#eff6ff;display:flex;align-items:center;justify-content:center;">
                    <i class="fas fa-{{ $asset->assigned_to_type === 'department' ? 'building' : 'user' }} text-primary"></i>
                </div>
                <div>
                    ...
                    <div class="fw-700">
                        ...
                        <div class="text-xs text-muted">{{ $asset->assignedEmployee->designation->name }}</div>
                        @endif
                        @endif
                    </div>
                </div>
            ...
            <div class="border-top pt-2 text-sm">
                ...
                <div class="mb-1"><i class="fas fa-building text-muted me-2"></i>{{ $asset->location_building }}</div>
                @endif
                @if($asset->location_floor)
                <div class="mb-1"><i class="fas fa-layer-group text-muted me-2"></i>Floor: {{ $asset->location_floor }}</div>
                @endif
                @if($asset->location_room_no)
                <div class="mb-1"><i class="fas fa-door-open text-muted me-2"></i>Room: {{ $asset->location_room_no }}</div>
                @endif
                @if($asset->assigned_on)
                <div class="mb-1"><i class="fas fa-calendar text-muted me-2"></i>Since: {{ $asset->assigned_on->format('d/m/Y') }}</div>
                @endif
            </div>
            @else
    ...
    <div class="card mb-3">
        ...
        <div class="card-body p-0">
            ...
            <div class="d-flex align-items-center gap-2 px-3 py-2 border-bottom">
                ...
                <div class="text-sm fw-600 text-success">₹{{ number_format($maint->maintenance_cost) }}</div>
                @endif
            </div>
            @empty
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="transaction_date"
                           class="form-control datepicker" value="{{ date('d/m/Y') }}" required>
                </div>
                <div class="mb-0">
