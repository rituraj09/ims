...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Header */
        .header { display: flex; align-items: center; border-bottom: 2px solid #000; padding-bottom: 10px; margin-bottom: 14px; gap: 14px; }
        .header-logo { width: 70px; height: 70px; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; font-size: 28px; border-radius: 50%; }
        .header-text { flex: 1; }
        .header-org { font-size: 14px; font-weight: 700; }
        .header-dept { font-size: 12px; color: #444; }
        .header-form { text-align: right; }
        .form-no { font-size: 14px; font-weight: 700; background: #000; color: #fff; padding: 4px 10px; border-radius: 4px; }
        .form-date { font-size: 11px; color: #555; margin-top: 4px; }

        /* Title */
        .form-title { text-align: center; font-size: 16px; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; border: 2px solid #000; padding: 8px; margin-bottom: 14px; background: #f5f5f5; }

        /* Section */
        .section { margin-bottom: 14px; }
        .section-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; background: #e0e0e0; padding: 4px 8px; border-left: 3px solid #000; margin-bottom: 6px; }

        /* Table */
        .info-table { width: 100%; border-collapse: collapse; }
        .info-table td { padding: 5px 8px; border: 1px solid #ccc; font-size: 12px; }
        .info-table td:first-child { font-weight: 700; width: 35%; background: #f9f9f9; }
        .info-table .val { font-size: 12px; }

        /* Two columns */
        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }

        /* Condition checklist */
        .condition-list { display: flex; gap: 8px; flex-wrap: wrap; }
        .condition-item { border: 1px solid #ccc; padding: 3px 8px; border-radius: 3px; font-size: 11px; }
        .condition-item.selected { background: #000; color: #fff; border-color: #000; font-weight: 700; }

        /* Remarks */
        .remarks-box { border: 1px solid #ccc; min-height: 50px; padding: 6px; }

        /* Signature */
        .signature-section { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 14px; margin-top: 20px; }
        .sig-box { border: 1px solid #ccc; padding: 10px; }
        .sig-line { border-top: 1px solid #000; margin-top: 40px; padding-top: 4px; font-size: 10px; text-align: center; }
        .sig-title { font-size: 10px; font-weight: 700; text-transform: uppercase; margin-bottom: 4px; color: #444; }
        .sig-name { font-size: 11px; font-weight: 700; }
        .sig-desig { font-size: 10px; color: #555; }

        /* Footer */
        .form-footer { border-top: 1px dashed #999; margin-top: 16px; padding-top: 8px; font-size: 10px; color: #777; display: flex; justify-content: space-between; }
...
<div class="page">
    ...
    <div class="header">
        ...
        <div class="header-text">
            <div class="header-org">Government of India</div>
    ...
    <div class="form-title">
        @if($assignment->transaction_type === 'handover')  Asset Handover Form
        @elseif($assignment->transaction_type === 'takeover') Asset Takeover Form
        @elseif($assignment->transaction_type === 'transfer') Asset Transfer Form
        @else {{ ucfirst($assignment->transaction_type) }} Form
        @endif
    </div>
    ...
    <div class="section">
        ...
        <table class="info-table">
            ...
            <tr>
                ...
                <td class="val">
                    <div class="condition-list">
                        ...
                        <span class="condition-item {{ $assignment->condition_at_handover === $cond ? 'selected' : '' }}">
                            {{ ucfirst($cond) }}
                        </span>
                        @endforeach
                    </div>
            ...
            <tr>
                ...
                <td class="val">{{ $assignment->asset->purchase_price ? '₹ ' . number_format($assignment->asset->purchase_price, 2) : '—' }}</td>
                <td>Current Value</td>
                <td class="val">{{ $assignment->asset->current_value ? '₹ ' . number_format($assignment->asset->current_value, 2) : '—' }}</td>
            </tr>
        </table>
    </div>
