...
@php
    ...
    try {
        $pendMaint = \App\Models\AssetMaintenance::whereIn('status', ['scheduled', 'in_progress'])->count();
    } catch (\Exception $e) {
        $pendMaint = 0;
    }
...
<aside class="sidebar" id="sidebar">
    ...
    <a href="{{ route('dashboard') }}" class="sb-brand">
        ...
        <span class="sb-brand-text">
            {{ \App\Models\Setting::get('general.app_name') ?? config('app.name') }}
        </span>
    </a>

    {{-- ── User Block ── --}}
    <div class="sb-user">
        <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="sb-user-img">
    ...
    <nav class="sb-nav" id="sbNav" aria-label="Main Navigation">
        ...
        @if ($can['assets_view'] || $can['categories_view'] || $can['maintenance_view'])
            ...
            {{-- Assets Menu --}}
            @if ($can['assets_view'])
                <button type="button" class="sb-link {{ $assetActive ? 'active' : '' }}" data-bs-toggle="collapse"
                    data-bs-target="#navAssets" aria-expanded="{{ $assetActive ? 'true' : 'false' }}"
                ...
                <ul class="sb-submenu collapse {{ $assetActive ? 'show' : '' }}" id="navAssets">
                    <li>
        ...
        @if ($can['reports_view'])
            ...
            <button type="button" class="sb-link {{ $reportActive ? 'active' : '' }}" data-bs-toggle="collapse"
                data-bs-target="#navReports" aria-expanded="{{ $reportActive ? 'true' : 'false' }}"
                aria-controls="navReports">
                <i class="fas fa-chart-bar sb-icon"></i>
                <span class="sb-label">Reports</span>
                <i class="fas fa-chevron-right sb-arrow"></i>
            </button>
            <ul class="sb-submenu collapse {{ $reportActive ? 'show' : '' }}" id="navReports">
                <li>
        ...
        @if ($can['users_view'] || $can['roles_manage'] || $can['settings_view'] || $isAdmin)
            ...
            @if ($can['settings_view'])
                <button type="button" class="sb-link {{ $settingActive ? 'active' : '' }}"
                    data-bs-toggle="collapse" data-bs-target="#navSettings"
                    aria-expanded="{{ $settingActive ? 'true' : 'false' }}" aria-controls="navSettings">
                    <i class="fas fa-gear sb-icon"></i>
                    <span class="sb-label">Settings</span>
                    <i class="fas fa-chevron-right sb-arrow"></i>
                </button>
                <ul class="sb-submenu collapse {{ $settingActive ? 'show' : '' }}" id="navSettings">
                    <li>
        ...
        <div style="margin: 6px 8px 0;">
            <form action="{{ route('logout') }}" method="POST" id="sidebarLogoutForm">
                ...
                <button type="button" onclick="document.getElementById('sidebarLogoutForm').submit()"
                    class="sb-link w-100" style="color:#f87171;">
